#!/usr/bin/env python3
"""
experiment_val_only_progen2.py — H1/H2 on val-only 150 proteins: ESM-2 vs ProGen2.

Reuses experiment_val_only_h1h2.locality_val but restricts to esm2 + progen2.
"""
...
from cpu_stage import DEFAULT_TOPK_FRAC

OUT_LW = Path(__file__).resolve().parent / "outputs_layerwise"

LAYERS = {"esm2": [0, 8, 16, 24, 32], "progen2": [0, 7, 14, 20, 26]}
MATCHED_PAIRS = [
    ("0%",    0,  0),
    ("25%",   8,  7),
    ("50%",  16, 14),
...
def main():
    ...
    ap.add_argument("--out", default="analysis_results_valonly_progen2")
    ...
    for depth_label, esm_l, pg_l in MATCHED_PAIRS:
        ...
        pg_all  = pd.read_csv(OUT_LW / "progen2" / f"layer_{pg_l}"  / "struct_seq_metrics.csv")
        esm_val = pd.read_csv(OUT_LW / "esm2"    / f"layer_{esm_l}" / "struct_seq_metrics_val.csv")
        pg_val  = pd.read_csv(OUT_LW / "progen2" / f"layer_{pg_l}"  / "struct_seq_metrics_val.csv")
        for split, (e, p) in [("all", (esm_all, pg_all)), ("val", (esm_val, pg_val))]:
            d_h1 = cohens_d(e.struct_delta.values, p.struct_delta.values)
            _, ph1 = mw_greater(e.struct_delta.values, p.struct_delta.values)
            ...
            d_h2 = cohens_d(p.seq_delta.values, e.seq_delta.values)
            _, ph2 = mw_greater(p.seq_delta.values, e.seq_delta.values)
    ...
    csv_out = out_dir / "val_only_h1h2_progen2.csv"
    df.to_csv(csv_out, index=False)
    print(f"\nWritten → {csv_out}")

    lines = ["ESM-2 vs ProGen2 H1/H2 — val-only (150 proteins) vs all (1500)\n",
             "=" * 72 + "\n"]
    for hyp in ("H1", "H2"):
    ...
    (out_dir / "val_only_h1h2_progen2.txt").write_text("".join(lines))
